...
<center><div class="container" align="center">
	...
	$x=$sosp*($p-1);
	include('dbcon.php');
	$s="select * from webtm_sanpham where idLoai=$loai limit $x,$sosp";
	...
	if(mysql_num_rows($kq)>0) { //neu so dong cua csdl >0
		...
		<div class="sp">
			...
			<div class="mua"><a href="dathang.php?idsp=<?php echo $d['idSP'] ?>&idloai=<?php echo $loai ?>">Mua</a></div>
        </div>
		<?php }
	} 
	else { ?>
   ...
   <div id="pt">
   <?php
   $s1="select* from webtm_sanpham where idLoai='$loai'";
   ...
   for($i=1;$i<=$tst;$i++) {
	   	if($i==$p)
			...
			<span id="index"><a style="text-decoration: none; color: white" href="index.php?p=<?php echo $i; ?>&idloai=<?php echo $d1['idLoai']; ?>"><?php echo $i; ?></a></span>
		<?php }
   }
   ?>
   </div>
